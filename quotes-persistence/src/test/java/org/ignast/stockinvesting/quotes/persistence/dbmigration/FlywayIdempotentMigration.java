    ...
    private void removeMigrationMetadata(final String version) {
        ...
        try {
            ...
            jdbcTemplate.execute(
                format(
                    """
                        DELETE FROM %s WHERE installed_rank IN (
                            SELECT last_installed_rank FROM (
                                SELECT MAX(installed_rank) AS last_installed_rank FROM %s
                            ) AS l
                        );""",
                    flywayMetadataTable,
                    flywayMetadataTable
            ...
            throw new IllegalStateException(
                format(
                    "Migration idempotency check failed: Supplied migration " +
                    "has not created expected migration record for version '%s' in metadata table '%s'",
                    version,
                    flywayMetadataTable
                )
            );
...
final class FlywayIdempotentMigrationTest {
    ...
    public void setup() {
        ...
        Flyway
            .configure()
            .dataSource(getDataSourceTo(MYSQL))
            .baselineOnMigrate(true)
            .target(ProductionDatabaseMigrationVersions.CURRENT);
    ...
    public void migrationShouldFailIfSuppliedMigrationDoesNotCreateExpectedMigrationMetadata() {
        assertThatExceptionOfType(IllegalStateException.class)
            .isThrownBy(() -> idempotentMigration.migrateTwice("2", new NoOpMysqlMigration()))
            .withMessage(
                format(
                    "Migration idempotency check failed: " +
                    "Supplied migration has not created expected migration record " +
                    "for version '%s' in metadata table '%s'",
                    "2",
