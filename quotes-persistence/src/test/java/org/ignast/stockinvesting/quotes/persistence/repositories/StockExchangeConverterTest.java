...
final class StockExchangeConverterTest {
    ...
    private final StockExchange nasdaqStockExchange = mock(
        StockExchange.class,
        e -> when(e.getMarketIdentifierCode()).thenReturn(nasdaqMic)
    );

    private final StockExchanges stockExchanges = mock(
        StockExchanges.class,
        s -> when(s.getFor(nasdaqMic)).thenReturn(nasdaqStockExchange)
    );

    @Test
    public void shouldPersistExchangeIdToDb() {
        assertThat(new StockExchangeConverter(stockExchanges).convertToDatabaseColumn(nasdaqStockExchange))
            .isEqualTo("XNAS");
    }

    @Test
    public void shouldRecreateExchangeFromIdInDb() {
        assertThat(new StockExchangeConverter(stockExchanges).convertToEntityAttribute("XNAS"))
            .isEqualTo(nasdaqStockExchange);
    }

    @Test
    public void shouldNotBeCreatedWithNullStockExchanges() {
        assertThatExceptionOfType(NullPointerException.class)
            .isThrownBy(() -> new StockExchangeConverter(null));
    }
}
